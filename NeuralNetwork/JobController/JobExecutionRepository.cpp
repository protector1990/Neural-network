...
namespace MFNeuralNetwork {
	namespace Data {
		...
		{
			// should lock this?
			sqlite3_bind_int64(_getAllJobExecsForJobStatement, 1, job.lock()->getId());
			return PreparedStatementResultGetter<JobExecution>::getResultFromPreparedStatement(_getAllJobExecsForJobStatement, this);
		}

		vector<shared_ptr<JobExecution>> JobExecutionsRepository::getUnfinishedJobExecutionsForJob(weak_ptr<Job> job)
		{
			sqlite3_bind_int64(_getUnfinishedJobExecsForJobStatement, 1, job.lock()->getId());
		...
		void JobExecutionsRepository::save(Entity * entity)
		{
			...
			sqlite3_bind_int64(_saveStatement, 2, jobExecution->getJob().lock()->getId());
		...
		{
			JobExecution* jobExecution = (JobExecution*)entity;
			sqlite3_bind_int64(_updateStatement, 1, jobExecution->getJob().lock()->getId());
